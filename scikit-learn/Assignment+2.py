...
def answer_one():
    ...
    for i, d in enumerate([1, 3, 6, 9]):
        ...
        X_test_poly = poly.transform(X_test.reshape(100,1))
...
def answer_two():
    ...
    for d in range(10):
        ...
        linreg = LinearRegression().fit(X_train_poly, y_train)
        r2_train[d] = linreg.score(X_train_poly, y_train)
        r2_test[d] = linreg.score(X_test_poly, y_test)
...
def answer_five():
    ...
    return list(X_train2.columns[indices[:5]]
)
answer_five()
...
def answer_seven():
    ...
    plt.ylabel('Score')
    plt.xscale('log')
    plt.plot(parameters, train_scores, parameters, test_scores)
